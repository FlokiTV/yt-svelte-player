<script lang="ts">
  import { getThumbnail } from "$lib/types/yt";
  import IconPlay from "$lib/icon/IconPlay.svelte";
  import IconSearch from "$lib/icon/IconSearch.svelte";
  export let currentSong;
  export let mp3URL;
  export let doOpenSearch: any;
  const getTitle = () => {
    return currentSong.title;
  };
  const getAuthor = () => {
    return currentSong.author.name;
  };
</script>

<div class="bg-white shadow-lg rounded-lg" style="max-width:400px;">
  <div class="flex flex-col">
    <div>
      <img
        class="w-full rounded"
        src={getThumbnail(currentSong)}
        alt="Album Pic"
      />
    </div>
    <div class="w-full p-8">
      <div class="flex justify-between">
        <div>
          <h3 class="text-2xl text-grey-darkest font-medium">
            {getTitle()}
          </h3>
          <p class="text-sm text-grey mt-1">{getAuthor()}</p>
        </div>
        <button
          class="text-black"
          on:click={() => {
            doOpenSearch();
          }}
        >
          <IconSearch />
          <!-- <svg
            class="w-6 h-6"
            fill="currentColor"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            ><path
              d="M10 3.22l-.61-.6a5.5 5.5 0 0 0-7.78 7.77L10 18.78l8.39-8.4a5.5 5.5 0 0 0-7.78-7.77l-.61.61z"
            /></svg
          > -->
        </button>
      </div>

      <!-- <div class="flex justify-between items-center mt-8">
        <div class="text-grey-darker">
          <svg
            class="w-8 h-8"
            fill="currentColor"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            ><path
              d="M6.59 12.83L4.4 15c-.58.58-1.59 1-2.4 1H0v-2h2c.29 0 .8-.2 1-.41l2.17-2.18 1.42 1.42zM16 4V1l4 4-4 4V6h-2c-.29 0-.8.2-1 .41l-2.17 2.18L9.4 7.17 11.6 5c.58-.58 1.59-1 2.41-1h2zm0 10v-3l4 4-4 4v-3h-2c-.82 0-1.83-.42-2.41-1l-8.6-8.59C2.8 6.21 2.3 6 2 6H0V4h2c.82 0 1.83.42 2.41 1l8.6 8.59c.2.2.7.41.99.41h2z"
            /></svg
          >
        </div>
        <div class="text-grey-darker">
          <svg
            class="w-8 h-8"
            fill="currentColor"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            ><path d="M4 5h3v10H4V5zm12 0v10l-9-5 9-5z" /></svg
          >
        </div>
        <div class="text-white p-8 rounded-full bg-green-500 shadow-lg">
          <IconPlay />
        </div>
        <div class="text-grey-darker">
          <svg
            class="w-8 h-8"
            fill="currentColor"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            ><path d="M13 5h3v10h-3V5zM4 5l9 5-9 5V5z" /></svg
          >
        </div>
        <div class="text-grey-darker">
          <svg
            class="w-8 h-8"
            fill="currentColor"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            ><path
              d="M5 4a2 2 0 0 0-2 2v6H0l4 4 4-4H5V6h7l2-2H5zm10 4h-3l4-4 4 4h-3v6a2 2 0 0 1-2 2H6l2-2h7V8z"
            /></svg
          >
        </div>
      </div> -->
    </div>
  </div>
  <div class="p-2">
    {#if mp3URL}
      <audio controls autoplay class="w-full">
        <source src={mp3URL} type="audio/mpeg" />
      </audio>
    {/if}
  </div>
  <!-- <div class="mx-8 py-4">
    <div class="flex justify-between text-sm text-grey-darker">
      <p>0:40</p>
      <p>4:20</p>
    </div>
    <div class="mt-1">
      <div class="h-1 bg-grey-dark rounded-full">
        <div class="w-1/5 h-1 bg-green-500 rounded-full relative">
          <span
            class="w-4 h-4 bg-red absolute pin-r pin-b -mb-1 rounded-full shadow"
          />
        </div>
      </div>
    </div>
  </div> -->
</div>
